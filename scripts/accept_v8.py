#!/usr/bin/env python3
"""v8 multi-viewport acceptance: departure board skin over the v6 console + v5 grid intact."""
...
async def main():
    ...
    async with async_playwright() as p:
        ...
        for w, h in VIEWPORTS:
            ...
            d = await page.evaluate("""(vh) => {
              ...
              return {
                heroH: Math.round(host.getBoundingClientRect().height), vh,
            ...
            d["errors"] = errors
            results[f"{w}x{h}"] = d
            await page.screenshot(path=f"/tmp/v8_accept_{w}x{h}.png")
            await ctx.close()
        await browser.close()
    ...
    for vp, d in results.items():
        checks = {
            ...
            "cream body": d["bodyBg"] == "rgb(245, 241, 230)",
            "no h overflow": not d["hScrollOverflow"],
            "no JS errors": len(d["errors"]) == 0,
        }
        fails = [k for k, v in checks.items() if not v]
        if fails: ok = False
        print(f"[{vp}] {'PASS' if not fails else 'FAIL: ' + str(fails)}  tminus={d['tminus']}  bodyBg={d['bodyBg']}  errors={d['errors'][:2]}")
    print(json.dumps({vp: {k: d[k] for k in ("heroH","vh","pxCount","teleShown","tminus","cards","firstCard","bodyBg")} for vp, d in results.items()}, ensure_ascii=False))
