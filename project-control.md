# Project Control Board

- 项目：zf-wang-personal-site
- 类型：个人作品集 / 履历 / 网站档案
- 当前模式：总控直做（未调用群内阶段 Bot）
- 域名：wangzifan.store
- 状态：SOURCE_READY · DNS_CURRENTLY_SPACESHIP_PARKING · CLOUDFLARE_AUTH_REQUIRED

## 线上现状（2026-07-12）
- 公共 DNS：Nameserver 为 `launch1.spaceship.net` / `launch2.spaceship.net`，尚未由 Cloudflare 托管。
- 根域 A 记录：`34.216.117.25`、`54.149.79.189`；HTTP 页面实际为 Spaceship `Parking Page`，不是本项目。
- 当前环境 Cloudflare Wrangler 未登录；本次已改由 GitHub Pages 部署。
- 已创建并推送新的可维护源码基线：`https://github.com/ZifanWang1996/wangzifan-personal-site`。这不覆盖现有线上停车页。
- 根域 A 已由公共 DNS 确认切换至 GitHub Pages：`185.199.108.153`、`185.199.109.153`、`185.199.110.153`、`185.199.111.153`。
- HTTP `wangzifan.store` 已实际返回本项目页面（GitHub 200）；HTTPS 暂因证书仍在签发而主机名不匹配，GitHub Pages 尚未允许强制 HTTPS。
- 已安排一次 30 分钟后的自动复查（任务 `654f917a9590`）；当前无需再次修改 DNS。

## 已确认（来自《王子凡简历2025》）
- 姓名：王子凡；2021 年开始工作。
- 当前：北京蓝天多维科技有限公司，项目经理（技术部），2024.06 至今。
- 经历：中电智能科技有限公司的项目管理 / 标准工程师及 C++ 软件工程师。
- 学历：燕山大学光学工程硕士（2021）；电子科学与技术学士（2018）。
- 能力：项目管理、工业控制、C++ / Qt、PLC、工业总线、跨架构适配、标准化。
- 认证/荣誉已留作后续扩展材料，未全部堆入首页。

## 已保留的隐私边界
- 未公开简历内的手机号、私人 QQ 邮箱、年龄或性别。
- 未提供公开联系邮箱、个人网站 URL、社交链接与域名。

## 当前任务
- [x] 信息架构与视觉方向
- [x] 静态实现
- [x] 简历事实提取与网页内容更新
- [ ] 网站记录补全（等待真实网站 URL/状态/简介）
- [ ] 公开联系方式确认
- [ ] 生产部署（等待域名与 Owner 发布授权）
