...
const count = new Element({ textContent: '45 / 45' });
const empty = new Element({ hidden: true });
const tools = new Element({ hidden: true });
const more = new Element({ textContent: '查看全部 45 条记录', hidden: true });
...
assert.equal(visibleRows().length, 22);
assert.equal(count.textContent, '22 / 45');
...
assert.equal(count.textContent, '1 / 45');
...
search.value = 'wowforever';
await search.emit('input');
assert.equal(visibleRows().length, 1);
assert.equal(visibleRows()[0].dataset.ledgerId, '45');

search.value = 'thedefiantwiki';
await search.emit('input');
assert.equal(visibleRows().length, 1);
assert.equal(visibleRows()[0].dataset.ledgerId, '44');
...
assert.equal(count.textContent, '0 / 45');
...
assert.equal(visibleRows().length, 45);
