...
const count = new Element({ textContent: '40 / 40' });
const empty = new Element({ hidden: true });
const tools = new Element({ hidden: true });
const more = new Element({ textContent: '查看全部 40 条记录', hidden: true });
...
assert.equal(visibleRows().length, 17);
assert.equal(count.textContent, '17 / 40');
...
assert.equal(count.textContent, '1 / 40');
...
assert.equal(count.textContent, '0 / 40');
