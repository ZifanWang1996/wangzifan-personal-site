...
const count = new Element({ textContent: '39 / 39' });
const empty = new Element({ hidden: true });
const tools = new Element({ hidden: true });
const more = new Element({ textContent: '查看全部 39 条记录', hidden: true });
...
assert.equal(visibleRows().length, 16);
assert.equal(count.textContent, '16 / 39');
...
assert.equal(count.textContent, '1 / 39');
...
assert.equal(count.textContent, '0 / 39');
...
assert.equal(visibleRows().length, 39);
