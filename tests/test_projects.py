...
def test_registry_assets_are_complete_lightweight_and_fixed_size():
    ...
    assert len(sources) == len(set(sources)) == 38
...
def test_homepage_truth_and_link_security_match_registry():
    ...
    # Hero status (1 link) + three latest cards (2 each) + three case links + 37 live ledger links.
    expected_safe_external_links = 1 + (3 * 2) + 3 + 37
    parser = AuditParser()
    parser.feed(html)
...
def test_workflow_builds_and_tests_before_exact_allowlist_upload():
    ...
    assert "for project_id in range(1, 39)" in manifest
