...
def test_v11_registry_has_complete_truthful_project_contract():
    ...
    assert [project["id"] for project in projects] == list(range(1, 43))
    assert len({project["url"] for project in projects}) == 42
    assert sum(project["status"] == "live" for project in projects) == 41
...
def test_build_v11_generates_truthful_identity_and_counts(tmp_path):
    ...
    assert 'data-status="releases">42 条公开记录' in html
    assert 'data-status="live">41 条在线记录' in html
...
def test_build_v11_generates_shared_assets_seo_and_privacy_page(tmp_path):
    ...
    assert release_list["numberOfItems"] == 42
    assert len(release_list["itemListElement"]) == 42
    structured_statuses = [
        item["item"]["additionalProperty"]["value"]
        for item in release_list["itemListElement"]
    ]
    assert structured_statuses.count("live") == 41
