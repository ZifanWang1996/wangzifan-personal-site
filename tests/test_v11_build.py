...
def test_v11_registry_has_complete_truthful_project_contract():
    ...
    assert [project["id"] for project in projects] == list(range(1, 35))
    assert len({project["url"] for project in projects}) == 34
    assert sum(project["status"] == "live" for project in projects) == 33
...
def test_build_v11_generates_truthful_identity_and_counts(tmp_path):
    ...
    assert 'data-hero-latest="34"' in html
    assert 'class="hero-latest-image"' not in html
    assert "工作台最近" in html
    assert "不叫“最佳作品”，这里只按上线时间排。" in html
    assert "GenVid Atlas" in html
    assert "2026-09-06" in html
    assert '<main id="main-content" tabindex="-1">' in html
    assert 'data-status="releases">34 条公开记录' in html
    assert 'data-status="live">33 条在线记录' in html
...
def test_build_v11_renders_latest_and_featured_case_studies(tmp_path):
    ...
    latest_positions = [html.index(f'data-latest-card="{project_id}"') for project_id in (34, 33, 32)]
...
def test_build_v11_closes_collaboration_method_ledger_and_contact_flow(tmp_path):
    ...
    ledger_positions = [html.index(f'data-ledger-id="{project_id}"') for project_id in range(34, 0, -1)]
    ...
    assert 'id="ledger-count" aria-live="polite">34 / 34' in html
    assert "这个页面收着 34 次公开上线" in html
...
def test_build_v11_generates_shared_assets_seo_and_privacy_page(tmp_path):
    ...
    assert release_list["numberOfItems"] == 34
    assert len(release_list["itemListElement"]) == 34
    structured_statuses = [
        item["item"]["additionalProperty"]["value"]
        for item in release_list["itemListElement"]
    ]
    assert structured_statuses.count("live") == 33
