...
def test_public_artifact_is_exact_allowlist(tmp_path):
    ...
    assert "public artifact: 44 files" in completed.stdout
    assert "candidate sha256=" in completed.stdout
    assert len(assert_artifact_closure(output)) == 44
...
def test_public_allowlist_rejects_renamed_project_image_even_when_count_stays_36(tmp_path):
    ...
    expected_image = source / "assets/projects/project-36.webp"
    expected_image.rename(source / "assets/projects/private-review-evidence.webp")

    with pytest.raises(FileNotFoundError, match="project-36.webp"):
        public_files(source)
